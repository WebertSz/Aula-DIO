    ...
    pokeApi.getPokemonMoreDetails(number).then((pokemonDetail = []) => {
        pokeModal.innerHTML = `
            <li class="pokemonOpenModal ${pokemonDetail.type}">
                <div id="fecharBotão">
                    <span>#${pokemonDetail.number.toString().padStart(3, "0")}</span>
                ...
                <div class="type">
                    ...
                    ${pokemonDetail.types.map((type) => `
                        <span class="type ${type}">${type}</span>`).join("")
                    }
                </div>
                <div class="pokemonImageModal">
                    <img class="pokemonImagem" src="${pokemonDetail.photo}" alt="${pokemonDetail.name}"/>
                </div>
                <div class="skills"> 
                    <span>Skills:</span>                
                    <span class="skillModal ${pokemonDetail.type}">${pokemonDetail.skill1}</span>
                      <span class="skillModal ${pokemonDetail.type}">${pokemonDetail.skill2}</span>
                </div>  
                <div >
                    <h3 >Attributes:</h3>
                </div>
